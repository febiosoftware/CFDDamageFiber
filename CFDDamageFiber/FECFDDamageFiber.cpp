...
#include <FEBioMech/FEFiberIntegrationScheme.h>
...
bool FECFDDamageFiber::Init()
{
	...
	FEFiberIntegrationScheme* rule = dynamic_cast<FEFiberIntegrationScheme*>(parent->ExtractProperty<FEFiberIntegrationScheme>());
...
bool FECFDDamageFiber::Validate()
{
	...
	// get the integration rule
	m_rule = parent->ExtractProperty<FEFiberIntegrationScheme>();
	...
	int nint = m_rule->IntegrationPoints();
...
mat3ds FECFDDamageFiber::FiberStress(FEMaterialPoint& mp, const vec3d& a0)
{
	...
	if (D != 0)
	{
		...
		{
			int nint = m_rule->IntegrationPoints();
			fp.m_D.resize (nint, 0.0);
			fp.m_Dp.resize(nint, 0.0);
		}
